...
//  my-cooler
//
//  Minimal AppleSMC wrapper for fan keys on Apple Silicon.
//
...
final class SMC {
    ...
    func readFloat(_ key: String) throws -> Float {
        let (info, bytes) = try read(key: key)
        return try decode(bytes: bytes, type: info.dataType)
    }
    ...
    private func decode(bytes: [UInt8], type: UInt32) throws -> Float {
        switch type {
        case kTypeFloat where bytes.count == 4:
            var v: Float = 0
            withUnsafeMutableBytes(of: &v) { dst in
                for i in 0..<4 { dst[i] = bytes[i] }
            }
            return v
        case kTypeFPE2 where bytes.count == 2:
            let v = (UInt16(bytes[0]) << 8) | UInt16(bytes[1])
            return Float(v) / 4.0
        case kTypeUInt8 where bytes.count >= 1:
